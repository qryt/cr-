...
parent.document.getElementsByName("shdm")[row].value=arr[0];
 
parent.document.getElementsByName("sl")[row].value="1";
parent.document.getElementsByName("shmx")[row].value=arr[1];
parent.document.getElementsByName("xs")[row].value= arr[2];
parent.document.getElementsByName("xsl")[row].value= arr[2];

parent.document.getElementsByName("gsf")[row].value= ff(arr[3]*arr[2],2);
parent.document.getElementsByName("gsfl")[row].value= arr[3];
parent.document.getElementsByName("clf")[row].value= arr[4];
parent.document.getElementsByName("clfl")[row].value= arr[4];
...
<TD  ><input type="hidden" name="d<%=k-1 %>" id="d<%=k-1 %>" value="${tsh.shdm }|${tsh.shmx }|<fmt:formatNumber value="${tsh.gs}" pattern="#0.00"></fmt:formatNumber>|<fmt:formatNumber value="${tsh.gsfl}" pattern="#0.00"></fmt:formatNumber>|<fmt:formatNumber value="${tsh.clf}" pattern="#0.00"></fmt:formatNumber>" /><font style="font-size:12px"><%=k-1 %></font></TD>
 ...
 <pg:index>
    ...
    <pg:prev ifnull="true">
      <% if (pageUrl != null) { %>
        <td align=right><A HREF="<%=pageUrl%>&pageNo=<%=pageNumber%>&id=<%=request.getParameter("id")%>&action=listsh">
    ...
    <pg:pages>
      ...
      <% } else { %>
        <td><A style="font-size:12px" HREF="<%=pageUrl%>&pageNo=<%=pageNumber%>&id=<%=request.getParameter("id")%>&action=listsh">
        <%=pageNumber%></A></td>
      <% } %>
    </pg:pages>
    <pg:next ifnull="true" >
